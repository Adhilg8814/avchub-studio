...
export async function buildContactSheet({ clipPath, outputPath, workDir, count = 5, columns = 1, frameWidth = 360, label = true } = {}) {
  ...
  for (let i = 0; i < times.length; i += 1) {
    ...
    const vf = [
      ...
      // open it and find every frame unlabelled and therefore uncitable.
      label && font ? `drawtext=expansion=none:fontfile='${font.replace(/:/g, "\\:")}':text='${text.replace(/:/g, "\\:")}':x=10:y=6:fontsize=20:fontcolor=white` : null
    ].filter(Boolean).join(",");
